...
controller.createProduct = async function (req, res) {
  try {
    ...
    return res
      .status(400)
      .send({ error: "Error creating product: " + error.message });
  }
};
...
controller.deleteProduct = async function (req, res) {
  ...
  try {
    ...
    return res.status(204).json({ message: "success", data: deleteProduct });
  } catch (error) {
    return res
      .status(500)
      .send({ error: "Error delete product: " + error.message });
  }
};
...
controller.getProducts = async function (req, res) {
  ...
  try {
    ...
    return res
      .status(400)
      .send({ error: "Error fetch products: " + error.message });
  }
};
